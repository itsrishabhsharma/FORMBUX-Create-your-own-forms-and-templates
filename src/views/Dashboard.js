...
class Dashboard extends React.Component{
  ...
  componentDidMount(){
    this.props.getForms()
    document.title="DASHBOARD | FORMBUX"
  }
  ViewForm(arr){
    if(arr.length>0)
  ...
  ceateForm(){
    ...
    APICall.post('forms/add',{name:this.state.name,fields:[]},Cookies.load('token')).then(res=>{
      this.setState({loading:false})
      if(res.data.status){
          this.props.getForms();
          this.props.history.push("/form/"+res.data.result)
      }
      else{
 ...
 render(){
  return (
    ...
    >
      {
        this.state.isNew===0?
        <div style={{display:'flex',flexDirection:'column',width:'100%',alignItems:'center',justifyContent:'space-between',height:'130px'}}>
        <PrimaryButton style={{width:'100%' }} onClick=   {()=>message.info('Not Available',0.4)}   size="xl">CHOOSE FROM TEMPLATES</PrimaryButton>
        <span style={{fontWeight:'bold',fontSize:'1.3em'}}>OR</span>
        <PrimaryButton style={{width:'100%' }} size="xl" bg="pink" onClick={()=>this.setState({isNew:2})}>CREATE NEW</PrimaryButton>
      </div>
                  ...
                  { this.props.all && this.props.all.length>0?<table className="items-center w-full bg-transparent border-collapse">
                    ...
                    <tbody>
                    {
                      this.props.all?this.props.all.map((itms,key)=>{
                          ...
                          <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-no-wrap p-4">
                            <EditOutlined onClick={()=>{this.props.history.push('/form/'+itms._id)}} style={{fontSize:'1.7em',cursor:'pointer'}} />
                            <Popconfirm
  ...
  >
                  ...
                  <CodeOutlined style={{fontSize:'1.5em',marginLeft:'8px',color:'grey',cursor:'pointer'}} onClick={()=>{itms.published.length>0?this.setState({activeForm:itms}):message.info('not published',0.6)}} />
                      ...
                      }):<span>Your forms will appear here</span>
                    }
                  ...
                  <span style={{fontWeight:'400',fontSize:'1.5em'}}>Your forms will appear here</span>
                  </div>}
